		...
		if (isObject(s)) {
			...
			Object.getOwnPropertyNames(s).forEach((e) => {
				...
				if (isObject(t)) {
					...
					((isArray(t.properties) && t.properties) || [e]).forEach(
						(t) => {
...
class DatasetController {
	...
	buildOrUpdateElements(t) {
		...
		(e._stacked = isStacked(e.vScale, e)),
			e.stack !== i.stack &&
				((a = !0), clearStacks(e), (e.stack = i.stack)),
	...
	updateRangeFromParsed(t, e, i, a) {
		...
		a &&
			i &&
			((a.values = i), (n = applyStack(a, s, this._cachedMeta.index))),
	...
	_resolveElementOptions(t, e = "default", i) {
		...
		const c = l.resolveNamedOptions(
			r,
			t,
			() => this.getContext(i, a, e),
			h
		);
		return (
			c.$shared &&
				((c.$shared = o),
				(s[n] = Object.freeze(cloneIfNotShared(c, o)))),
			c
		);
	}
	...
	_calculateBarValuePixels(t) {
		...
		return (
			...
			m === e.getPixelForValue(r) &&
				((t = (sign(p) * e.getLineWidthForValue(r)) / 2),
				(m += t),
				(p -= t)),
...
class BubbleController extends DatasetController {
	...
	updateElements(e, i, a, s) {
		...
		var { sharedOptions: l, includeOptions: h } = this._getSharedOptions(
				i,
				s
			),
			c = r.axis,
	...
	updateElements(t, e, i, a) {
		...
		for (p = e; p < e + i; ++p) {
			...
			u &&
				(v.options =
					d ||
					this.resolveDataElementOptions(p, m.active ? "active" : a)),
				(g += f),
...
class LineController extends DatasetController {
	...
	updateElements(e, i, t, a) {
		...
		var { sharedOptions: h, includeOptions: c } = this._getSharedOptions(
				i,
				a
			),
			d = n.axis,
...
class PolarAreaController extends DatasetController {
	...
	updateElements(a, t, e, s) {
		...
		for (g = t; g < t + e; g++) {
			...
			(u = e),
				n &&
					(o.animateScale && (i = 0), o.animateRotate && (t = e = d));
			var m = {
				x: h,
				y: c,
				innerRadius: 0,
		...
		evaluateInteractionItems(n, t, r, function (t, e, i) {
			...
			(o && !s) ||
				...
				(h || n.isPointInArea(a) || s) &&
					((s = d(r, a)) < u
						? ((c = [{ element: t, datasetIndex: e, index: i }]),
						  (u = s))
						: s === u &&
						  c.push({ element: t, datasetIndex: e, index: i })));
		}),
		c
	);
}
...
function buildStacks(t) {
	...
	for (const n of t) {
		...
		if (i && STATIC_POSITIONS.includes(a)) {
			const r =
				e[i] || (e[i] = { count: 0, placed: 0, weight: 0, size: 0 });
			r.count++, (r.weight += s);
		}
	}
	return e;
	...
	return (
		t.isHorizontal()
			? ((g = _alignStartEnd(a, n, o)),
			  (p = isObject(i)
					? ((m = i[(f = Object.keys(i)[0])]),
					  c[f].getPixelForValue(m) + l - e)
					: "center" === i
...
class Scale extends Element {
			...
			: ((e = (t = this._getLabelSizes()).widest.width),
			  ...
			  (s.offset ? this.maxWidth / r : a / (r - 1)) < e + 6 &&
					...
					(h = toDegrees(
						Math.min(
							Math.asin(
								_limitValue((t.highest.height + 6) / a, -1, 1)
							),
		...
		c &&
			...
			d
				? ((t.width = this.maxWidth),
				  (t.height = getTickMarkLength(h) + c))
				: ((t.height = this.maxHeight),
				  (t.width = getTickMarkLength(h) + c)),
			o.display &&
				this.ticks.length &&
				(({
					first: l,
				...
				if (isArray(g))
					for (d = 0, u = g.length; d < u; ++d)
						(_ = g[d]),
							isNullOrUndef(_) ||
								isArray(_) ||
								((x = _measureText(a, m.data, m.gc, x, _)),
								(b += v));
	...
	_computeLabelItems(t) {
		...
		"top" === a
			...
			: "left" === a
			? ((c = this._getYAxisLabelAlignment(i)),
			  (_ = c.textAlign),
			  (x = c.x))
			: "right" === a
			? ((c = this._getYAxisLabelAlignment(i)),
	...
	drawGrid(t) {
		...
		if (e.display)
			for (s = 0, n = i.length; s < n; ++s) {
				var o = i[s];
				e.drawOnChartArea &&
					r({ x: o.x1, y: o.y1 }, { x: o.x2, y: o.y2 }, o),
					e.drawTicks &&
						r(
							{ x: o.tx1, y: o.ty1 },
							{ x: o.tx2, y: o.ty2 },
...
class Chart {
	...
	buildOrUpdateControllers() {
		...
		let a, t;
		for (
			this._removeUnreferencedMetasets(), a = 0, t = i.length;
			a < t;
			a++
		) {
			var s = i[a];
	...
	_updateDatasets(i) {
		if (
			...
			for (let t = 0, e = this.data.datasets.length; t < e; ++t)
				this._updateDataset(
					t,
					isFunction(i) ? i({ datasetIndex: t }) : i
				);
	...
	updateHoverStyle(t, e, i) {
		...
		for (
			...
			const l =
				(n = t[r]) && this.getDatasetMeta(n.datasetIndex).controller;
	...
	t.beginPath(),
		n
			? (t.arc(r, o, d, f, (n = (f + m) / 2)),
			  ...
			  t.lineTo(x.x, x.y),
			  0 < c &&
					((n = rThetaToXY(e, f, r, o)),
					t.arc(n.x, n.y, c, i - HALF_PI, f)))
			: (t.moveTo(r, o),
			  (_ = Math.cos(f) * d + r),
			  (g = Math.sin(f) * d + o),
			  t.lineTo(_, g),
...
function pathSegment(t, e, i, a) {
	...
	for (u = 0; u <= l; ++u)
		(g = s[(o + (d ? l - u : u)) % r]).skip ||
			(c ? (t.moveTo(g.x, g.y), (c = !1)) : h(t, p, g, d, n.stepped),
			(p = g));
...
function lttbDecimation(a, s, n, t, e) {
	...
	for (r[l++] = a[h], c = 0; c < i - 2; c++) {
		...
		for (u = -1, i = f; i < x; i++)
			(g =
				0.5 *
				Math.abs((b - t) * (a[i].y - _) - (b - a[i].x) * (e - _))) >
				u && ((u = g), (d = a[i]), (p = i));
		(r[l++] = d), (h = p);
	}
	return (r[l++] = a[e]), r;
...
function _getBounds(i, a, s, t) {
	if (!t) {
		...
		return (
			"angle" === i &&
				((t = _normalizeAngle(t)), (e = _normalizeAngle(e))),
...
class Legend extends Element {
	...
	_fitCols(t, a, s, n) {
		...
		return (
			this.legendItems.forEach((t, e) => {
				var { itemWidth: t, itemHeight: i } = calculateItemSize(
					s,
					a,
					r,
					t,
		...
		this.legendItems.forEach((t, e) => {
			...
			(e = e),
				...
				C
					...
					? ((d = _.lineHeight),
					  (M.y += calculateLegendItemHeight(t, d)))
					: (M.y += O);
		}),
...
function calculateItemHeight(t, e, i) {
	let a = t;
	return (a =
		"string" != typeof e.text ? calculateLegendItemHeight(e, i) : a);
}
		...
		return (
			(l = this.isHorizontal()
				...
				: ((o =
						"left" === n.position
							? ((h = i + t),
							  (c = _alignStartEnd(r, a, e)),
							  -0.5 * PI)
							: ((h = s - t),
							  (c = _alignStartEnd(r, e, a)),
							  0.5 * PI)),
				  a - e)),
		...
		return (
			each(t, (t) => {
				...
				pushOrConcat(
					e.before,
					splitNewlines(
						invokeCallbackWithFallback(i, "beforeLabel", this, t)
					)
				),
					pushOrConcat(
						e.lines,
					...
					pushOrConcat(
						e.after,
						splitNewlines(
							invokeCallbackWithFallback(i, "afterLabel", this, t)
						)
					),
					s.push(e);
			}),
		...
		return (
			...
			each(e, (t) => {
				...
				i.push(invokeCallbackWithFallback(e, "labelColor", this, t)),
					...
					r.push(
						invokeCallbackWithFallback(e, "labelTextColor", this, t)
					);
			}),
			(this.labelColors = i),
	...
	getTickLimit() {
		...
		return (
			e
				? 1e3 <
						(i =
							Math.ceil(this.max / e) -
							Math.floor(this.min / e) +
							1) &&
				  (console.warn(
...
function updateLimits(t, e, i, a, s) {
	...
	a.start < e.l
		...
		: a.end > e.r &&
		  ((r = (a.end - e.r) / n), (t.r = Math.max(t.r, e.r + r))),
		s.start < e.t
	...
	determineDataLimits() {
		...
		(n && r) ||
			...
			("ticks" === t.bounds && "labels" === t.ticks.source) ||
				o(this.getMinMax(!1))),
			(a =
				isNumberFinite(a) && !isNaN(a) ? a : +e.startOf(Date.now(), i)),
			(s =
				isNumberFinite(s) && !isNaN(s)
					? s
	...
	initOffsets(t = []) {
		...
		this.options.offset &&
			...
			(e =
				1 === t.length
					? 1 - a
					: (this.getDecimalForValue(t[1]) - a) / 2),
	...
	getDataTimestamps() {
		...
		if (this._normalized && a.length)
			return (this._cache.data =
				a[0].controller.getAllParsedValues(this));
...
class TimeSeriesScale extends TimeScale {
	...
	getValueForPixel(t) {
		...
		return interpolate(
			this._table,
			t * this._tableRange + this._minPos,
			!0
		);
	}
}
